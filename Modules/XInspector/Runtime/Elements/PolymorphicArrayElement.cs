...
#endif
    public class PolymorphicArrayElement : XInspectorElement, IExpandableElement, IPropertyAttributeElement
    {
        private const float HeaderHeight = 26f;
        private const float RowMinHeight = 22f;
        private const float HandleWidth = 16f;
        private const float TypeFieldWidth = 220f;
        ...
        private const string NoTypesChoice = "No Types";

        private static ClipboardData s_Clipboard;

        private PolymorphicListAttribute polymorphicListAttribute;
        ...
        private readonly DropdownField createTypeDropdown;
        private readonly List<Type> candidateTypes = new();
        ...
        public PolymorphicArrayElement()
        {
            ...
            createTypeDropdown = new DropdownField
            {
                style =
                {
                    width = TypeFieldWidth,
                    minWidth = TypeFieldWidth,
                    marginRight = 4
                }
            };
            createTypeDropdown.RegisterValueChangedCallback(evt => createTypeDropdown.tooltip = evt.newValue);
            ...
            AddToClassList("polymorphic-array-element");
            Add(title);
        }

        public void SetPropertyAttribute(PropertyAttribute attribute)
        {
            polymorphicListAttribute = attribute as PolymorphicListAttribute;
        }
        ...
        private void CreateElements()
        {
            ...
            for (int i = 0; i < candidateErrors.Count; i++)
            {
                elementsContent.Add(CreateMessageRow(candidateErrors[i], true));
            }

            if (Value == null || Length <= 0)
            {
                selectedIndex = -1;
                elementsContent.Add(CreateMessageRow("Empty", false));
                return;
            }

            for (int i = 0; i < Length; i++)
            {
                elementsContent.Add(CreateRow(i));
            }
        ...
        private VisualElement CreateMessageRow(string text, bool isError)
        {
            return new Label(text)
            {
                style =
                {
                    ...
                    unityTextAlign = TextAnchor.MiddleCenter,
                    backgroundColor = isError ? new Color(0.28f, 0.13f, 0.12f, 1f) : new Color(0.15f, 0.15f, 0.15f, 1f),
                    color = isError ? new Color(0.95f, 0.45f, 0.40f, 1f) : new Color(0.45f, 0.45f, 0.45f, 1f),
        ...
        private VisualElement CreateRow(int index)
        {
            ...
            if (rowType == null)
            {
                field.Add(CreateNullValueElement());
            }
            else
            {
                XInspectorElement elementDrawer = XInspector.CreateDrawerForMemberType(rowType, 0);
                elementDrawer.BindTo(rowType, GetElementLabel(index, rowType), () => GetElementValue(rowIndex), value => SetElementValue(rowIndex, value));
        ...
        private VisualElement CreateNullValueElement()
        {
            return new Label(NoneChoice)
            {
                style =
                {
                    minHeight = RowMinHeight,
        ...
        private static string GetElementLabel(int index, Type type)
        {
            return $"Element {index} ({GetTypeShortName(type)})";
        }
        ...
        {
            return InsertDefaultElement(Length);
        }

        private bool InsertDefaultElement(int index)
        {
            if (!TryGetSelectedCreateType(out Type selectedType))
            {
                return false;
            }

            InsertElement(index, CreateElementInstance(selectedType));
            return true;
        }
        ...
        private bool TryGetSelectedCreateType(out Type selectedType)
        {
            selectedType = null;
            if (candidateTypes.Count == 0)
            {
                Debug.LogError($"[PolymorphicList] {BoundVariableType} 没有可创建的候选类型");
                return false;
            }

            string selectedTypeName = createTypeDropdown.value;
            selectedType = candidateTypes.FirstOrDefault(type => GetTypeDisplayName(type) == selectedTypeName);
            if (selectedType == null)
            {
                selectedType = candidateTypes[0];
                createTypeDropdown.SetValueWithoutNotify(GetTypeDisplayName(selectedType));
            }
        ...
        private void BuildHeaderContextMenu(ContextualMenuPopulateEvent evt)
        {
            evt.menu.AppendAction(
                ...
                _ => candidateTypes.Count > 0 ? DropdownMenuAction.Status.Normal : DropdownMenuAction.Status.Disabled);
            evt.menu.AppendAction(
                "Clear",
                _ =>
                {
        ...
        private void BuildElementContextMenu(ContextualMenuPopulateEvent evt, int index)
        {
            ...
            evt.menu.AppendAction(
                ...
                _ => CanPasteElement(index) ? DropdownMenuAction.Status.Normal : DropdownMenuAction.Status.Disabled);
            ...
            evt.menu.AppendAction(
                ...
                _ =>
                {
                    if (InsertDefaultElement(index + 1))
                    {
                        selectedIndex = index + 1;
                        Refresh();
                    }
        ...
        private bool CanPasteElement(int index)
        {
            return s_Clipboard != null
                   && s_Clipboard.IsElement
                   && IsCompatibleValue(s_Clipboard.Element)
                   && IsSameElementType(GetElementValue(index), s_Clipboard.Element);
        }

        private bool TryPasteElement(int index)
        {
            if (index < 0 || index >= Length || !CanPasteElement(index))
            {
                return false;
            }
        ...
        private bool CanPasteCollection()
        {
            return s_Clipboard != null
                   && !s_Clipboard.IsElement
                   && s_Clipboard.Elements.All(IsCompatibleValue);
        }
        ...
        private bool IsCompatibleValue(object value)
        {
            return value == null || elementType.IsInstanceOfType(value);
        }

        private static bool IsSameElementType(object currentValue, object pastedValue)
        {
            return currentValue?.GetType() == pastedValue?.GetType();
        }

        private void ResolveCandidateTypes()
        {
            candidateTypes.Clear();
            candidateErrors.Clear();

            IEnumerable<Type> sourceTypes = polymorphicListAttribute != null && polymorphicListAttribute.types.Length > 0
                ? polymorphicListAttribute.types
                : CollectAutoCandidateTypes();

            var seenTypes = new HashSet<Type>();
            foreach (Type type in sourceTypes)
            {
                if (!CanCreateElementType(type, out string error))
                {
                    if (polymorphicListAttribute != null && polymorphicListAttribute.types.Length > 0)
                    {
                        string message = $"[PolymorphicList] {error}";
                        candidateErrors.Add(message);
            ...
            candidateTypes.Sort((left, right) => string.Compare(GetTypeDisplayName(left), GetTypeDisplayName(right), StringComparison.Ordinal));
            if (candidateTypes.Count == 0)
            {
                string message = $"[PolymorphicList] {elementType} 没有可创建的候选类型";
                candidateErrors.Add(message);
                Debug.LogError(message);
            }
        }

        private IEnumerable<Type> CollectAutoCandidateTypes()
        {
            foreach (Assembly assembly in AppDomain.CurrentDomain.GetAssemblies())
            {
                foreach (Type type in GetAssemblyTypes(assembly))
                {
                    if (type.IsClass && elementType.IsAssignableFrom(type))
                    {
                        yield return type;
                    }
                }
        ...
        private bool CanCreateElementType(Type type, out string error)
        {
            if (type == null)
            {
                error = $"{Name} 配置了空类型";
            ...
            {
                error = $"{GetTypeDisplayName(type)} 不能赋值给 {GetTypeDisplayName(elementType)}";
                return false;
            }

            if (type.IsAbstract || type.IsInterface)
            {
                error = $"{GetTypeDisplayName(type)} 不是具体类型";
                return false;
            }

            if (type.ContainsGenericParameters)
            {
                error = $"{GetTypeDisplayName(type)} 不能是未闭合泛型类型";
                return false;
            }

            if (type.GetConstructor(Type.EmptyTypes) == null)
            {
                error = $"{GetTypeDisplayName(type)} 缺少 public 无参构造";
                return false;
            }
        ...
        private void UpdateCreateTypeDropdown()
        {
            List<string> choices = candidateTypes.Count > 0
                ? candidateTypes.Select(GetTypeDisplayName).ToList()
                : new List<string> { NoTypesChoice };
            string currentValue = createTypeDropdown.value;
            string selectedValue = choices.Contains(currentValue) ? currentValue : choices[0];
            createTypeDropdown.choices = choices;
            createTypeDropdown.SetValueWithoutNotify(selectedValue);
            createTypeDropdown.tooltip = selectedValue;
            addLabel.SetEnabled(candidateTypes.Count > 0);
            addLabel.style.color = candidateTypes.Count > 0
        ...
        private sealed class ClipboardData
        {
            ...
            public static ClipboardData CaptureElement(PolymorphicArrayElement owner, int index)
            {
            ...
            public static ClipboardData CaptureCollection(PolymorphicArrayElement owner)
            {
