...
namespace XFramework.Animation
{
    ...
    public sealed class XAnimationActor : MonoBehaviour
    {
        ...
        public void Initialize(TextAsset animationAsset, Animator animator)
        {
            ...
            {
                throw new XAnimationException($"{nameof(XAnimationActor)} requires an XAnimation TextAsset.");
            }

            if (animator == null)
            {
                throw new XAnimationException($"{nameof(XAnimationActor)} requires an Animator.");
            }
        ...
        {
            throw new XAnimationException("XAnimationActor.Play(string clipKey) has been removed. Use PlayClip(clipName, channelName, ...) instead.");
        }
